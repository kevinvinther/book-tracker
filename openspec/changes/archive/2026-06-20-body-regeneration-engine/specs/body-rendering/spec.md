...
The system SHALL provide a `renderSeriesBody(series, index)` function that renders a Series entity as a markdown string with: an `# {name}` heading, and an ordered numbered list of linked works (by `series_position` ascending, unpositioned works last). Only works that exist SHALL be listed — no placeholder entries.
...
- **WHEN** a series has no linked works
