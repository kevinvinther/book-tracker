...
## Purpose
A separate production Compose definition that orchestrates the image with `NODE_ENV=production`, a data-only volume mount, an automatic restart policy, and a container healthcheck.
