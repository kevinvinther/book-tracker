...
## Purpose
A multi-stage production image that compiles the server, builds the client, runs the compiled server with production dependencies only, takes its port from the environment, and shuts down cleanly on termination signals.
