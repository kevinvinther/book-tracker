...
Each copy card SHALL display the copy's format (from its edition), condition, ownership status badge, location, and acquisition source — all currently available fields. Read-through status, note count, and loan badges SHALL NOT be displayed (deferred to a future change once those backends exist).
